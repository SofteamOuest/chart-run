        ...
        container('helm') {
            ...
            stage('upgrade') {
                withCredentials([string(credentialsId: 'registry_url', variable: 'registry_url')]) {
                    ...
                    def options = "--namespace ${params.env} --set-string env=${platform} --set-string image.tag=${params.image} meltingpoc-charts/${params.chart} --set ingress.hosts[0]=${params.chart}.k8.wildwidewest.xyz,ingress.tls[0].hosts[0]=${params.chart}.k8.wildwidewest.xyz"
